...
class ModelWrapper(LightningModule):
    ...
    def test_step(self, batch, batch_idx):
        ...
        # print('near_value shape---------------------', near_value.shape)
        # print('far_value shape---------------------', far_value.shape)
        # print(' near_value---------------------', near_value)
        # print('far_value---------------------', far_value)
        # print('batch["context"][0]--', batch["context"][0])
        # print('batch["context"][1]-- ', batch["context"][1])
        ...
        for index_pose, interpolated_pose in enumerate(interpolated_poses):
            ...
            if self.test_cfg.save_image:
                name = [index_pose,index_pose,index_pose,index_pose]
                # for index, color in zip(batch["target"]["index"][0], images_prob):
                for index, color in zip(name, images_prob):
                    save_image(color, path / scene / f"color_re10k/{index:0>4}.png")
